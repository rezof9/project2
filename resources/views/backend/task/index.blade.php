                                    ...
                                    <div class="card draggable-handle draggable" data-id="{{ $task->id_task }}" data-status="Pending">
                                        <div class="card-body">
                                            ...
                                            <div class="profile-skills mt-1 mb-1">
                                                <a href="javascript:void();;" class="btn btn-primary light btn-xs mb-1"> File  <b>{{ $task->fileTasks_count ??"0" }}</b> </a>
                                                <a href="javascript:void();;" class="btn btn-primary light btn-xs mb-1"> Komentar  <b>{{ $task->messageTasks_count ??"0" }}</b></a>
                                    
                                            </div>
                                            <div class="mt-2">
                                                <h5>Assigned to:</h5>
                                                <ul class="list-group">
                                                    @forelse($task->assignmentTasks as $assignment)
                                                        @if($assignment->pegawai)
                                                            <li class="list-group-item">{{ $assignment->pegawai->nama }}</li> <!-- Menampilkan nama pegawai -->
                                                        @endif
                                                    @empty
                                                        <li class="list-group-item text-muted">Belum di-assign</li> <!-- Menampilkan pesan jika tidak ada pegawai yang terdaftar -->
                                                    @endforelse
                                                </ul>
                                            </div>
