                ...
                <div class="card">
                  <div class="card-body">
                    <div class="list-product-header">
                      <div>
                        <div class="light-box"><a data-bs-toggle="collapse" href="#collapseProduct" role="button" aria-expanded="false" aria-controls="collapseProduct"><i class="filter-icon show" data-feather="filter"></i><i class="icon-close filter-close hide"></i></a></div><a class="btn btn-primary" href="{{ route ('about.create') }}"><i class="fa fa-plus"></i>Add Program</a>
                      </div>
                            ...
                            <tbody>
                                @foreach ($abouts as $index => $about)
                                    <tr>
                                        ...
                                        <td>{{ $about->title ?? 'No Name' }}</td>
                                        <td>{!! strip_tags($about->description) !!}</td>
                                        <td>
                                            @if ($about->rating)
                                            @for ($i = 1; $i <= 5; $i++)
                                            <span class="star {{ $i <= $about->rating ? 'filled' : '' }}">&#9733;</span>
                                                @endfor
                                            @else
                                            @endif
                                        </td>
                                        ...
                                        <p class="created-at">Created on: {{ $about->created_at->format('d M Y') }}</p> <!-- Format tanggal -->
                                    </div>
                                        </td>
                                        <td>
                                            <form action="{{ route('about.index', $program->id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                                @csrf
                                                @method('DELETE')
